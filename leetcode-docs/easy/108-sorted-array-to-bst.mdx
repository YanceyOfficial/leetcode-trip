...
var helper = function (nums, left, right) {
  ...
  const mid = Math.floor((left + right) / 2) // 求中间索引
  const root = new TreeNode(nums[mid]) // 构建当前节点
  root.left = helper(nums, left, mid - 1) // 构建左子树
  root.right = helper(nums, mid + 1, right) // 构建右子树
