...
这道题可以转化成 [0-1 背包问题](/algorithm-design/dynamic-programming/knapsack#0-1-背包). 先给 `nums` 求和记为 `weight`, 因此这道题就变成了: 给你背包容量为 `weight / 2` 和 `n` 个物品, 每个物品的重量为 `nums[i]`, 现在让你装物品, 是否存在一种装法, 能够恰好将背包装满.
