...
- 时间复杂度: O(n<sup>2</sup>), 其中 n 表示日程安排的数量. 由于每次在进行预订时, 都需要遍历所有已经预订的行程安排.
...
- 空间复杂度: O(nlogC), 其中 n 为日程安排的数量。由于该解法采用的为动态线段树, 线段树的最大深度为 logC, 每次预订最多会在线段树上增加 logC 个节点, 因此空间复杂度为 O(nlogC), 在此 C 取固定值 10<sup>9</sup>.
