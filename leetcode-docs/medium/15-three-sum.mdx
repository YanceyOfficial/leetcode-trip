...
function twoSumTarget(nums, target) {
  ...
  while (low < high) {
    ...
    if (sum < target) {
      // sum < target 说明左指针需要往右移动, 但为了防止 nums[low] 与 leftVal 相同,
      // 需要通过 while 循环将相同的值过滤掉
      while (low < high && nums[low] === leftVal) {
        low++
      }
    } else if (sum > target) {
      // sum > target 同理
      while (low < high && nums[high] === rightVal) {
        high--
      }
    } else {
      res.push([leftVal, rightVal])

      // 即便取到了一对 leftVal 和 rightVal, 也要左右指针往中间夹, 来过滤重复的值
      while (low < high && nums[low] === leftVal) {
        low++
      }
      while (low < high && nums[high] === rightVal) {
