...
var myAtoi = function (s) {
  ...
  while (i < n && /[0-9]/.test(s[i])) {
    ...
    // 注意这块比较有意思, 思路是通过 INT_MAX 逆过来(INT_MAX / 10), 来推断 res 是否溢出
    // 首先注意 INT_MAX 是 2147483647
    // 如果当前的 res 大于 INT_MAX / 10, 说明溢出了, 需要返回 INT_MAX 或者 INT_MIN
    // 如果当前的 res 等于 INT_MAX / 10 后取整, 也就是 214748364, 但 num 仍大于 INT_MAX 取余, (其实就是 INT_MAX 的最后一位, 也就是 7), 也说明溢出了
    if (
      res > INT_MAX / 10 ||
