...
This problem is very similar to [560. Subarray Sum Equals K](/leetcode/medium/560-subarray-sum-equals-k). The r difference is that we're looking for subarrays whose sum is divisible by `k`, rather than equal to `k`.
...
We need to handle negative remainders correctly using: `r = Math.abs(preSum % k)`
...
2. Iterate through the array:
   ...
   - Calculate remainder: `r = Math.abs(preSum % k)`
